...
but that you can call directly from the class. Static methods can only be appended to the parent class,
and not to subclasses. For example:

Take the Date class, for example — you can both create Date instances to represent whatever date you want,
and call static methods, like Date.now() which returns the current date, directly from the class. The .now()
